 ...
 */
class ServerSyncData : public virtual NIE::InformationElement
{
public:
    ServerSyncData(const QUrl& uri = QUrl())
      : SimpleResource(uri), NIE::InformationElement(uri, QUrl::fromEncoded("http://www.example.com/sync#ServerSyncData", QUrl::StrictMode)) {
    }

    ServerSyncData(const SimpleResource& res)
      : SimpleResource(res), NIE::InformationElement(res, QUrl::fromEncoded("http://www.example.com/sync#ServerSyncData", QUrl::StrictMode)) {
    }
