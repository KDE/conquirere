#ifndef _NIE_DATASOURCE_H_
#define _NIE_DATASOURCE_H_
...
public:
    DataSource(const QUrl& uri = QUrl())
      : SimpleResource(uri) {
        addType(QUrl::fromEncoded("http://www.semanticdesktop.org/ontologies/2007/01/19/nie#DataSource", QUrl::StrictMode));
    }

    DataSource(const SimpleResource& res)
      : SimpleResource(res) {
        addType(QUrl::fromEncoded("http://www.semanticdesktop.org/ontologies/2007/01/19/nie#DataSource", QUrl::StrictMode));
    }

    DataSource& operator=(const SimpleResource& res) {
