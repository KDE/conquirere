...
EncoderLaTeX::EncoderLaTeX()
        : d(new EncoderLaTeX::EncoderLaTeXPrivate)
{
    d->buildCharMapping();
...
QString EncoderLaTeX::decode(const QString & text)
{
    ...
    for (QList<EncoderLaTeXPrivate::CharMappingItem>::iterator cmit = d->charMapping.begin(); cmit != d->charMapping.end(); ++cmit)
...
QString EncoderLaTeX::encode(const QString &text, const QChar &replace)
{
    ...
    for (QList<EncoderLaTeXPrivate::CharMappingItem>::iterator it = d->charMapping.begin(); it != d->charMapping.end(); ++it)
