...
void ZoteroDownload::attachmentDownloadFinished()
{
    ...
    // zotero redirects the file download to the amazon cloud service, this takes care of those redirects
    ...
    foreach(SyncDetails sd, m_tmpUserDeleteRequest) {
        ...
        if(deleteThem) {
            ...
            if( syncDataType == SYNC::Note()) {
                ...
                mainResource = sd.syncResource.property( SYNC::publication() ).toResource();
            }
...
void ZoteroDownload::writeNewSyncDetailsToNepomuk(Entry *localData, const QString &id, const QString &etag, const QString &updated)
{
    ...
    if(localData->type() == QLatin1String("note")) {
        ...
        serverSyncData.setPublication( publication.uri() );
        ...
        if(!refUri.isEmpty()) {
            ...
            serverSyncData.setPublication( reference.uri() );
